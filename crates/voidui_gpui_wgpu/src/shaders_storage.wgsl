...
@group(2) @binding(0) var<storage, read> b_gradient_data: array<vec4<u32>>;
fn load_gradient_word(index: u32) -> vec4<u32> { return b_gradient_data[index]; }

@group(0) @binding(2) var<storage, read> b_spatial_data: array<vec4<u32>>;
fn load_spatial_word(index:u32) -> vec4<u32> { return b_spatial_data[index]; }
