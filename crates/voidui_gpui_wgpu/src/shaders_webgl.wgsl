...
@group(2) @binding(0) var t_gradient_data: texture_2d<u32>;
fn load_gradient_word(index: u32) -> vec4<u32> {
    let width = textureDimensions(t_gradient_data).x;
    return textureLoad(t_gradient_data, vec2<i32>(i32(index % width), i32(index / width)), 0);
}

@group(0) @binding(2) var t_spatial_data: texture_2d<u32>;
fn load_spatial_word(index:u32) -> vec4<u32> {
    let w=textureDimensions(t_spatial_data).x;
    return textureLoad(t_spatial_data,vec2<i32>(i32(index%w),i32(index/w)),0);
}
