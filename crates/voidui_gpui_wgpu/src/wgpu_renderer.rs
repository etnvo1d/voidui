...
struct WgpuResources {
    ...
    spatial_data: InstanceData,
    spatial_capacity: u64,
    globals_bind_group: wgpu::BindGroup,
    path_globals_bind_group: wgpu::BindGroup,
    instance_data: InstanceData,
    gradient_data: InstanceData,
...
impl WgpuRenderer {
        ...
        W: HasWindowHandle + HasDisplayHandle + std::fmt::Debug + Send + Sync + Clone + 'static,
    {
        ...
        // WGPU owns a cloned handle source (e.g. Arc<winit::window::Window>).
        // This keeps the native window alive even if the caller drops its copy.
    ...
    fn new_internal(
        ...
        let (spatial_data, spatial_capacity) =
            Self::create_gradient_data(&device, uses_webgl_instance_data, 256);
        let globals_bind_group = Self::global_binding(
            &device,
        ...
        let path_globals_bind_group = Self::global_binding(
            &device,
            &bind_group_layouts.globals,
            &globals_buffer,
            path_globals_offset,
        ...
        // A single word satisfies unused shader bindings. Real gradient storage
        // grows only when a scene contains gradients, independently of the viewport.
        let (gradient_data, gradient_capacity) =
            Self::create_gradient_data(&device, uses_webgl_instance_data, 16);
    ...
    fn create_bind_group_layouts(
        ...
        let mut gradient_entry = instance_data_entry;
        gradient_entry.visibility = wgpu::ShaderStages::FRAGMENT;
        let gradients = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("gradients_layout"),
            entries: &[gradient_entry],
        });
    ...
    /// Keep AppKit resize and Metal presentation in the same transaction.
    /// Call on the window thread before acquiring a frame; disable after the
    /// synchronous resize draw. Other backends do not need this hook.
    #[cfg(target_os = "macos")]
    pub fn set_presents_with_transaction(&self, enabled: bool) {
        // SAFETY: the guarded HAL surface is only borrowed. Its layer is never
        // destroyed or replaced; the mutex serializes presentation properties.
        // WGPU samples this property at acquire time and performs the required
        // waitUntilScheduled + drawable.present sequence for transactional frames.
        if let Some(surface) = unsafe { self.resources().surface.as_hal::<wgpu::hal::api::Metal>() }
        {
            let layer = surface.render_layer().lock();
            layer.setPresentsWithTransaction(enabled);
            // A hidden/occluded window must not block the UI thread indefinitely
            // in nextDrawable. The caller handles unsuccessful draws with backoff.
    ...
    /// Notify the window system only after rendering succeeds and immediately
    /// before presentation. Wayland uses this point to request a frame callback.
    pub fn draw_with_present_callback(
        &mut self,
        scene: &Scene,
    ...
    fn write_frame_uniforms(&self) {
        ...
        let globals = GlobalParams {
            ...
            srgb_framebuffer: u32::from(self.surface_config.format.is_srgb()),
        };
    ...
    /// Render a finished scene to tightly packed RGBA8 pixels at the current viewport size.
    ...
    pub fn render_to_rgba(&mut self, scene: &Scene) -> Result<Vec<u8>> {
        ...
        self.record_frame(scene, &texture.create_view(&Default::default()))?;
        ...
        let buffer = resources.device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("scene_capture_readback"),
    ...
    fn draw_paths_to_intermediate(
...

        {
            ...
            pass.set_bind_group(2, &resources.gradient_bind_group, &[]);
        ...
        *instance_offset = offset + allocation_size;
        ...
        if !data.is_empty() {
            match &resources.instance_data {
                ...
                InstanceData::Texture { .. } => {
                    Self::write_instance_texture(resources, &resources.instance_data, offset, data)
                }
            }
        }
    ...
    fn create_gradient_data(
        ...
        if webgl {
            ...
            (
                InstanceData::Storage(device.create_buffer(&wgpu::BufferDescriptor {
                    label: Some("gradient_data"),
                    size: capacity,
                    usage: wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_DST,
                    mapped_at_creation: false,
                })),
    ...
    fn global_binding(
        ...
        device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                ...
                wgpu::BindGroupEntry {
                    binding: 2,
                    resource: match spatial {
                        InstanceData::Storage(b) => b.as_entire_binding(),
                        InstanceData::Texture { view, .. } => {
                            wgpu::BindingResource::TextureView(view)
                        }
                    },
    ...
    /// Reuse the spatial upload allocation until a larger scene needs it.
    fn write_spatial(&mut self, words: &[[u32; 4]]) -> Result<()> {
        if words.is_empty() {
            return Ok(());
        }
        ...
        anyhow::ensure!(
            required <= self.max_instance_data_size,
            "spatial data exceeds device limits"
        );
        if required > self.resources().spatial_capacity {
            let capacity = required
                .next_power_of_two()
                .min(self.max_instance_data_size);
            let webgl = self.uses_webgl_instance_data;
            let path_offset = self.path_globals_offset;
            let gamma_offset = self.gamma_offset;
            let r = self.resources_mut();
            let (data, capacity) = Self::create_gradient_data(&r.device, webgl, capacity);
            r.globals_bind_group = Self::global_binding(
                &r.device,
                &r.bind_group_layouts.globals,
                &r.globals_buffer,
                0,
                gamma_offset,
                &data,
            );
            r.path_globals_bind_group = Self::global_binding(
                &r.device,
                &r.bind_group_layouts.globals,
                &r.globals_buffer,
                path_offset,
            ...
            r.spatial_data = data;
            r.spatial_capacity = capacity;
        }
        let r = self.resources();
        match &r.spatial_data {
            InstanceData::Storage(buffer) => r.queue.write_buffer(buffer, 0, bytes),
            InstanceData::Texture { .. } => {
                Self::write_instance_texture(r, &r.spatial_data, 0, bytes)
            }
        }
        Ok(())
    }
    ...
    fn write_gradients(&mut self, words: &[[u32; 4]]) -> Result<()> {
        if words.is_empty() {
            return Ok(());
        }
        let bytes = bytemuck::cast_slice(words);
        ...
        anyhow::ensure!(
            required <= self.max_instance_data_size,
            "gradient data exceeds device limits"
        );
        if required > self.resources().gradient_capacity {
            let capacity = required
                .next_power_of_two()
                .min(self.max_instance_data_size);
            let webgl = self.uses_webgl_instance_data;
            let resources = self.resources_mut();
            let (data, capacity) = Self::create_gradient_data(&resources.device, webgl, capacity);
            resources.gradient_bind_group = Self::gradient_binding(
                &resources.device,
                &resources.bind_group_layouts.gradients,
                &data,
            );
            resources.gradient_data = data;
            resources.gradient_capacity = capacity;
        }
        let resources = self.resources();
        match &resources.gradient_data {
            InstanceData::Storage(buffer) => resources.queue.write_buffer(buffer, 0, bytes),
            InstanceData::Texture { .. } => {
                Self::write_instance_texture(resources, &resources.gradient_data, 0, bytes)
            }
        }
        Ok(())
    }
        ...
        W: HasWindowHandle + HasDisplayHandle + std::fmt::Debug + Send + Sync + Clone + 'static,
    {
        ...
        let surface = if needs_new_context {
            ...
            *gpu_context.borrow_mut() = None;
            ...
            // The host schedules recovery with backoff. Sleeping here blocks
            // the UI thread and unrelated windows during device recovery.
        ...
        let config = WgpuSurfaceConfig {
            size: crate::Size {
                width: crate::DevicePixels(self.surface_config.width as i32),
                height: crate::DevicePixels(self.surface_config.height as i32),
            },
