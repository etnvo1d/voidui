...
// Identity entries never read spatial storage. General affine transforms retain
// local interpolation for borders, gradients, shadows, and image sampling.
fn spatial_point(p:vec2<f32>, id:u32)->vec2<f32> {
    if id==0u {return p;}
    let a=bitcast<vec4<f32>>(load_spatial_word(id));
    let b=bitcast<vec4<f32>>(load_spatial_word(id+1u));
    return vec2<f32>(dot(a.xy,p)+a.z,dot(b.xy,p)+b.z);
}
...
/// Hsla to encoded sRGBA conversion.
...
fn blend_color(color: vec4<f32>, alpha_factor: f32) -> vec4<f32> {
    ...
    // HSL and CSS colors are encoded sRGB. UNORM targets store these bytes
    // directly; sRGB targets need linear shader output to avoid double encoding.
    var rgb = color.rgb;
    // Uniform control flow avoids evaluating the transfer function on UNORM
    // targets, which are the normal fast path for native windows.
    if (globals.srgb_framebuffer != 0u) { rgb = srgb_to_linear(rgb); }
...
fn prepare_gradient_color(tag: u32, color_space: u32,
    ...
    if (tag == 0u || tag == 2u || tag == 3u) {
        ...
        // HSL channels describe encoded sRGB, not linear-light RGB.
        ...
        if (color_space == 0u) {
            // sRGB
            // Encoded sRGB is already the requested interpolation space.
...
// The color ramp is shared across the quad. Duplicate positions preserve hard
// stops; upper-bound search selects the last stop at an exact discontinuity.
fn css_gradient(index: u32, position: vec2<f32>) -> vec4<f32> {
    let header = load_gradient_word(index);
    let geometry = bitcast<vec4<f32>>(load_gradient_word(index+1u));
    let count = header.z;
    if (count == 0u) { return vec4<f32>(0.0); }
    let first = bitcast<vec4<f32>>(load_gradient_word(index+5u)).x;
    let last = bitcast<vec4<f32>>(load_gradient_word(index+5u+(count-1u)*2u)).x;
    var t: f32;
    var pixel_span = 0.0;
    let tile = bitcast<vec4<f32>>(load_gradient_word(index+3u));
    var point = position;
    if (all(tile.zw > vec2<f32>(0.0))) {
        // The CSS initial background-origin is padding-box, while clipping is
        // border-box. Repeat the image into the portion behind the border.
        let offset = position - tile.xy;
        point = tile.xy + offset - floor(offset/tile.zw)*tile.zw;
    }
    let relative = point - geometry.xy;
    if (header.x == 0u) {
        let length = bitcast<vec4<f32>>(load_gradient_word(index+2u)).x;
        pixel_span = (abs(geometry.z)+abs(geometry.w))/max(length,1e-20);
        t = select(last, dot(relative, geometry.zw)/max(length, 1e-20)+0.5, length > 0.0);
    } else if (header.x == 1u) {
        pixel_span = 1.0/max(min(geometry.z,geometry.w),1e-20);
        t = select(last, length(relative / max(geometry.zw,vec2<f32>(1e-20))), all(geometry.zw > vec2<f32>(0.0)));
    } else {
        pixel_span = 1.0/max(length(relative)*2.0*M_PI_F,1e-20);
        let turns = (atan2(relative.x,-relative.y)-geometry.z)/(2.0*M_PI_F);
        t = turns-floor(turns);
    }
    if (header.y != 0u && last-first < pixel_span) {
        let average=bitcast<vec4<f32>>(load_gradient_word(index+4u));
        if (average.a<=0.0) {return vec4<f32>(0.0);}
        return vec4<f32>(average.rgb/average.a,average.a);
    }
    if (header.y != 0u && last > first) { t = first + (t-first) - floor((t-first)/(last-first))*(last-first); }
    var low = 0u;
    var high = count;
    loop {
        if (low >= high) { break; }
        let middle = low + (high-low)/2u;
        let offset = bitcast<vec4<f32>>(load_gradient_word(index+5u+middle*2u)).x;
        if (offset <= t) { low = middle+1u; } else { high = middle; }
    }
    let a = select(0u,low-1u,low>0u);
    let b = min(low,count-1u);
    let oa = bitcast<vec4<f32>>(load_gradient_word(index+5u+a*2u)).x;
    let ob = bitcast<vec4<f32>>(load_gradient_word(index+5u+b*2u)).x;
    let ca = bitcast<vec4<f32>>(load_gradient_word(index+6u+a*2u));
    let cb = bitcast<vec4<f32>>(load_gradient_word(index+6u+b*2u));
    let f = select(0.0,clamp((t-oa)/max(ob-oa,1e-20),0.0,1.0),ob>oa);
    let premul = mix(ca,cb,f);
    if (premul.a <= 0.0) { return vec4<f32>(0.0); }
...
fn gradient_color(background: Background, position: vec2<f32>, bounds: Bounds,
    ...
    switch (background.tag) {
        ...
        case 1u: {
            ...
            let center_to_point = position - bounds.origin - half_size;
            let gradient_length = dot(abs(direction),bounds.size);
            let position_on_line = dot(center_to_point,direction)/max(gradient_length,1e-20)+0.5;
            let span = stop1_percentage-stop0_percentage;
            var t = clamp((position_on_line-stop0_percentage)/max(span,1e-20),0.0,1.0);
            if (span <= 0.0) { t = select(0.0,1.0,position_on_line >= stop1_percentage); }

            let premul = mix(color0,color1,t);
            var mixed = vec4<f32>(0.0);
            if (premul.a > 0.0) { mixed = vec4<f32>(premul.rgb/premul.a,premul.a); }
            switch (background.color_space) {
                default: { background_color = mixed; }
                case 1u: { background_color = linear_to_srgba(oklab_to_linear_srgb(mixed)); }
            }
        }
        case 4u: { return css_gradient(background.gradient_index, position); }
        case 2u: {
            // pattern slash
...
struct QuadVarying {
    ...
    @location(7) @interpolate(flat) spatial_id: u32,
...
fn vs_shadow(@builtin(vertex_index) vertex_id: u32, @builtin(instance_index) instance_id: u32) -> ShadowVarying {
    ...
    out.spatial_id = shadow.spatial_id;
    out.position = spatial_position(out.local_position, shadow.spatial_id);
...
fn fs_shadow(input: ShadowVarying) -> @location(0) vec4<f32> {
    ...
    var alpha: f32;
    if (any(shadow.bounds.size <= vec2<f32>(0.0))) {
        alpha = 0.0;
    ...
    if (shadow.inset != 0u) {
        ...
        // An outer CSS shadow never shows through the element's border box,
        // including when the background is transparent.
        let distance = quad_sdf(input.local_position, shadow.element_bounds, shadow.element_corner_radii);
        alpha *= saturate(0.5 + distance);
    }
...
fn vs_path_rasterization(@builtin(vertex_index) vertex_id: u32) -> PathRasterizationVarying {
    ...
    out.spatial_id = v.spatial_id;
    out.position = spatial_position(out.local_position, v.spatial_id);
...
fn vs_underline(@builtin(vertex_index) vertex_id: u32, @builtin(instance_index) instance_id: u32) -> UnderlineVarying {
    ...
    out.spatial_id = underline.spatial_id;
    out.position = spatial_position(out.local_position, underline.spatial_id);
...
fn vs_mono_sprite(@builtin(vertex_index) vertex_id: u32, @builtin(instance_index) instance_id: u32) -> MonoSpriteVarying {
    ...
    out.local_position = transpose(sprite.transformation.rotation_scale) * (sprite.bounds.origin + unit_vertex * sprite.bounds.size) + sprite.transformation.translation;
    out.spatial_id = sprite.spatial_id;
    out.position = spatial_position(out.local_position, sprite.spatial_id);
...
fn fs_poly_sprite(input: PolySpriteVarying) -> @location(0) vec4<f32> {
    ...
    let low = vec2<f32>(sprite.tile.bounds.origin) + vec2<f32>(0.5);
    let high = low + vec2<f32>(sprite.tile.bounds.size) - vec2<f32>(1.0);
    var texel = input.tile_position * atlas_size;
    if ((sprite.pad & 3u) == 2u) {
        let scale = max(round(sprite.bounds.size / vec2<f32>(sprite.tile.bounds.size)), vec2<f32>(1.0));
        let base = floor(texel);
        // Most of an enlarged source pixel samples its center. Only the
        // subpixel-width boundary interpolates with its neighbor.
        let f = fract(texel);
        texel = base + min(f * scale, vec2<f32>(0.5))
                     + max((f - vec2<f32>(1.0)) * scale + vec2<f32>(0.5), vec2<f32>(0.0));
    }
    // Clamp at texel centers to prevent linear filtering into neighboring atlas tiles.
    let uv = clamp(texel, low, high) / atlas_size;
    var sample = textureSample(t_sprite, s_sprite, uv);
    if ((sprite.pad & 3u) == 1u) {
        sample = textureLoad(t_sprite, vec2<i32>(clamp(floor(texel), floor(low), floor(high))), 0);
    }
    ...
    let distance = quad_sdf(input.local_position, sprite.rounded_bounds, sprite.corner_radii);
