...
-- Capture this file's directory before package callbacks change context.
local portsdir = path.join(os.scriptdir(), "ports")
...
package("voidui-sdl3")
    ...
    on_install(function (package)
        os.cp(path.join(portsdir, "sdl3.lua"), "xmake.lua")
        os.cp(path.join(portsdir, "sdl3-linux.h"), "SDL_build_config_linux.h")
...
package("voidui-freetype")
    ...
    add_links("freetype")
    on_install(function (package)
        os.cp(path.join(portsdir, "freetype.lua"), "xmake.lua")
        import("package.tools.xmake").install(package)
    end)
    on_test(function (package)
...
package("voidui-harfbuzz")
    ...
    add_links("harfbuzz")
    on_install(function (package)
        os.cp(path.join(portsdir, "harfbuzz.lua"), "xmake.lua")
        import("package.tools.xmake").install(package)
    end)
    on_test(function (package)
