...
pub struct Scene {
    ...
    pub(crate) image_textures: Vec<(usize, crate::ImageTexture)>,
}
...
impl Scene {
    ...
    pub fn insert_primitive(&mut self, primitive: impl Into<Primitive>) {
        ...
        primitive.set_spatial_id(self.spatial_id);
        let local_bounds = primitive
            .bounds()
            .intersect(&primitive.content_mask().bounds);
        let clipped_bounds = if self.spatial_id == 0 {
            local_bounds
        } else {
            self.spatial
                .transform(self.spatial_id)
        ...
        match &mut primitive {
            ...
            Primitive::Path(path) => {
                path.spatial_bounds = (self.spatial_id != 0).then_some(clipped_bounds);
                path.order = order;
    ...
    pub fn replay(&mut self, range: Range<usize>, prev_scene: &Scene) {
        ...
        let mut copied_spaces = std::collections::HashMap::new();
        let start = prev_scene
            .image_textures
            .partition_point(|(i, _)| *i < range.start);
        let end = prev_scene
            .image_textures
            .partition_point(|(i, _)| *i < range.end);
        let offset = self.paint_operations.len();
        self.image_textures.extend(
            prev_scene.image_textures[start..end]
                .iter()
                .map(|(i, t)| (offset + i - range.start, t.clone())),
        );
            ...
            match operation {
                PaintOperation::Primitive(primitive) => {
                    ...
                    if let Primitive::Quad(quad) = &mut primitive
                        && quad.background.tag == crate::color::BackgroundTag::CssGradient
                    {
                        let start = quad.background.gradient_index as usize;
                        let count = prev_scene.gradient_data[start][2] as usize;
                        quad.background.gradient_index = self
                            .gradient_data
                            .len()
                            .try_into()
                            .expect("gradient index overflow");
                        self.gradient_data.extend_from_slice(
                            &prev_scene.gradient_data
                                [start..start + crate::gradient::GRADIENT_HEADER_WORDS + 2 * count],
                        );
                    }
                    self.spatial_id = self.spatial.copy_space(
                        &prev_scene.spatial,
...
impl Primitive {
    fn set_spatial_id(&mut self, id: u32) {
        match self {
            ...
            Self::Underline(p) => p.spatial_id = id,
            Self::MonochromeSprite(p) => p.spatial_id = id,
            Self::SubpixelSprite(p) => p.spatial_id = id,
            Self::PolychromeSprite(p) => p.spatial_id = id,
            Self::Surface(p) => p.spatial_id = id,
        }
    }
    fn spatial_id(&self) -> u32 {
        match self {
            ...
            Self::Underline(p) => p.spatial_id,
            Self::MonochromeSprite(p) => p.spatial_id,
            Self::SubpixelSprite(p) => p.spatial_id,
            Self::PolychromeSprite(p) => p.spatial_id,
            Self::Surface(p) => p.spatial_id,
        }
    }
...
impl Shadow {
    /// Include Gaussian tails in culling and paint-order overlap tests. Inset
    /// shadows paint the element, even if their hole lies entirely outside it.
    pub fn paint_bounds(&self) -> Bounds<ScaledPixels> {
        if self.inset != 0 {
            self.element_bounds
        } else {
            self.bounds.dilate(self.blur_radius * 3.0)
        }
    }
}
...
pub struct PolychromeSprite {
    ...
    pub content_mask: ContentMask<ScaledPixels>,
    pub corner_radii: Corners<ScaledPixels>,
    /// Rounded clipping follows the element content box, including object-fit cropping.
...
pub struct Path<P: Clone + Debug + Default + PartialEq> {
    ...
    pub(crate) spatial_bounds: Option<Bounds<P>>,
...
impl Path<Pixels> {
    ...
    pub fn scale(&self, factor: f32) -> Path<ScaledPixels> {
        Path {
            ...
            spatial_bounds: self.spatial_bounds.map(|b| b.scale(factor)),
            vertices: self
                .vertices
                .iter()
    ...
    T: Clone + Debug + Default + PartialEq + PartialOrd + Add<T, Output = T> + Sub<Output = T>,
{
    ...
    pub fn clipped_bounds(&self) -> Bounds<T> {
        self.spatial_bounds
            .clone()
            .unwrap_or_else(|| self.bounds.intersect(&self.content_mask.bounds))
    }
}
